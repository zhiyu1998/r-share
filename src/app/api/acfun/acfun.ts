...
const parseVideoName = (videoInfo: acfunVideoInfo) => {
...
const parseVideoNameFixed = (videoInfo: acfunVideoInfo) => {
...
export async function parseUrl(videoUrlAddress: string) {
	...
	const urlM3u8s = representations.map((d: any) => d.url);
...
export async function parseM3u8(m3u8Url: string) {
	...
	const m3u8FullUrls = m3u8RelativeLinks.map(
		(d: string) => m3u8Prefix + "/" + d,
	);
	/** aria2c下载的文件名，就是取url最后一段，去掉末尾url参数(?之后是url参数) */
	const tsNames = m3u8RelativeLinks.map((d: string) => d.split("?")[0]);
	/** 文件夹名，去掉文件名末尾分片号 */
...
export async function downloadM3u8Videos(
	...
	const strDownloadParamFiles = m3u8FullUrls.map(async (d, i) => {
		return new Promise((resolve, reject) => {
			const writer = fs.createWriteStream(path.join(outPath, `${i}.ts`));
			axios
				.get(d, {
					headers: {
						"User-Agent":
...
export async function mergeAcFileToMp4(
	tsNames: string[],
	FullFileName: string,
	...
	const concatStrs = tsNames.map(
		(d, i) =>
			`file ${path.resolve(FullFileName, i + ".ts").replace(/\\/g, "/")}`,
	);
