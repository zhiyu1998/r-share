...
const tr = Array.from(table).reduce(
	(o, c, i) => Object.assign(o, { [c]: i }),
	{},
);
 ...
 */
function bv2AV(bv: number) {
	return (
		// @ts-ignore
		(new Uint8Array(6).reduce((r, _, i) => r + tr[bv[s[i]]] * 58 ** i, 0) -
			add) ^
		xor
	);
}
 ...
 */
function av2BV(av: number) {
	return new Uint8Array(6)
		.reduce((r, _, i) => {
			r.splice(
				s[i],
				1,
				table[Math.floor((((av ^ xor) + add) / 58 ** i) % 58)],
			);
			return r;
